...
#[tracing::instrument]
pub async fn init(cli: Cli, config: Config, action_tx: UnboundedSender<Action>) -> Result<()> {
    KEYS.first.store(false, Ordering::Relaxed);
    *ACTION_TX.write().await = action_tx;
    let mut client = CONFIGURATION.write().await;

    let mut builder = reqwest::ClientBuilder::new().danger_accept_invalid_certs(
        cli.accept_invalid_certificate || config.network.accept_danger,
    );
